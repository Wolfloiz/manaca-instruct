...
| Degenerate outputs (FR-009) | new `src/dataset_filters/quality.py::is_degenerate_output`: `re.search(r"(.)\1{5,}\|(\S+\s)\2{3,}", text)` or, for `len(text) >= 30`, `len(set(text)) / len(text) < 0.15` | catches the 37 rows found by the audit's regex (e.g. `canarim-274024`, `-301183`) across all categories |
