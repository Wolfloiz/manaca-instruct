...
- [ ] T043 [US2] (You) **Decision needed**: T042 missed SC-002 on all 5 categories, which is the trigger condition for FR-005's single budgeted iteration round (`qlora-v2`) — adjust `configs/train.yaml` within research.md §2's ranges and repeat T038–T042 once, or accept qlora-v1 as final and document the shortfall as a known limitation. Given real, measurable learning occurred (0%→15-31% everywhere) and zero forgetting, this may be a case where more epochs/data would help rather than a fundamentally broken run — worth a deliberate choice, not a default
