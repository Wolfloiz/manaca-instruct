...
- [ ] T025 [US1] (You) Run `src/grading/review_cli.py` against `eval/results/baseline.jsonl` to grade every `manual_review` row; confirm at least one `grupo_a` prompt shows the base model not following the instruction (Acceptance Scenario 2) — **not done**: 0/72 `manual_review` rows graded yet (rule-based already confirms the acceptance scenario, but the forgetting-check baseline for SC-003 needs these scored before qlora-v1 can be compared against it)
