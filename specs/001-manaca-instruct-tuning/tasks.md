...
- [ ] T027 [P] [US2] (Agent 1, in `../manaca-instruct-agent1-dataset`) Implement `src/dataset_filters/grammar_rewriting.py`: pull `dominguesm/alpaca-data-pt-br`, filter/curate to `grammar_correction` + `rewriting` examples matching the `InstructionExample` schema (data-model.md)
- [ ] T028 [P] [US2] (Agent 2) Implement `src/dataset_filters/open_ended_tasks.py`: pull `dominguesm/Canarim-Instruct-PTBR-Dataset` (and `alpaca-data-pt-br` as needed), filter/curate to `simplification` + `summarization` + `classification` examples matching the `InstructionExample` schema
- [ ] T029 [US2] (Agent 1) Implement `src/prepare_dataset.py`: orchestrates T027 + T028's filter modules, deduplicates against `data/eval/grupo_a_prompts.jsonl`/`grupo_b_prompts.jsonl` (no `(instruction, input)` overlap, per data-model.md), writes `data/train.jsonl` + `data/validation.jsonl` totaling **3,000–5,000** examples roughly evenly split across the 5 categories — depends on T027, T028
- [ ] T030 [US2] (Agent 1) Open PR #4 (`agent1-dataset` → `001-manaca-instruct-tuning`) with T027, T029
- [ ] T031 [US2] (Agent 2) Open PR #5 (`agent2-eval` → `001-manaca-instruct-tuning`) with T028
- [ ] T032 [US2] (You) Code-review and merge PR #4 and PR #5 (merge PR #5 first so T029's dependency on T028 is satisfiable if you re-run prepare_dataset locally, though both are needed regardless of merge order to build successfully)
- [ ] T033 [US2] (You) Run `python src/prepare_dataset.py`; spot-check the resulting `train.jsonl`/`validation.jsonl` for Portuguese correctness and category balance; approve, or send specific examples back to Agent 1/2 for a follow-up fix commit
- [ ] T034 [P] [US2] (Agent 3, in `../manaca-instruct-agent3-infra`) Implement `src/train_qlora.py`: QLoRA fine-tuning entrypoint — `menezesbruno/manaca-1b-base` + `configs/train.yaml` + `data/train.jsonl` → adapter checkpoint under `adapters/<run-id>/`
- [ ] T035 [P] [US2] (Agent 3) Implement `src/merge_adapter.py`: merges a trained LoRA adapter into a standalone model under `models/merged/<run-id>/`
- [ ] T036 [US2] (Agent 3) Open PR #6 (`agent3-infra` → `001-manaca-instruct-tuning`) with T034–T035
- [ ] T037 [US2] (You) Code-review (use `/code-review high` — training-script correctness bugs are easy to miss by eye) and merge PR #6
