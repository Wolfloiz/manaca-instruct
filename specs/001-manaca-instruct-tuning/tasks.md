...
**Open question found while reviewing T039/T040's rule_based scores**: `grammar_correction`'s scorer (edit-distance ≥0.9 vs. a single reference) may be too strict for this task — qlora-v1's actual output for one prompt (`"os meninos foram à aula hoje."`) correctly fixed subject-verb agreement but paraphrased content (escola→aula, ontem→hoje), landing at ratio 0.787, just under threshold, scored 0. Both qlora-v1 and the official model show this same pattern (0% despite visible grammar fixes in spot checks) — worth a manual read-through of the "failed" rows before accepting 0% as the real result, or revisiting the threshold/scoring approach for this category specifically.
