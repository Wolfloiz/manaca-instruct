...
**Update 2026-09-17 (feature 002 merged, PR #30)**: the model this feature publishes is decided — `qlora-v3b` with `repetition_penalty 1.1`, adopted under a blind protocol against qlora-v2 and the official release (`eval/results/final-table.md`, `eval/results/adoption-decision.md`). The FR-005 iteration (qlora-v2) is superseded by 002's candidates; the 70% target (FR-004/SC-002) remains unmet and is documented as a known limitation in `MODEL_CARD.md`. Remaining here: T051 (Dell G3 benchmark, no physical access), T058–T059 (publish + verification), T061–T063 (sign-off, merge to `main`, tag).

**Update 2026-09-15**: T015 done (project `.venv`, CUDA confirmed on the RTX 5050) and T024 done (real baseline: 104 rows against `manaca-1b-base`, 0/32 rule-based pass rate — confirms Acceptance Scenario 1). Also: the `evaluate.py` `_load_model`/`_generate` seams got wired to real `transformers`/`peft` (PR #11), plus two real bugs found from that first real run — a missing prompt-template applied at eval time (PR #12) and a `torch_dtype`→`dtype` deprecation (PR #13). Separately, the `train_qlora.py`/`merge_adapter.py`/`quantize.py`/`benchmark.py`/`publish.py` seams were wired to real `bitsandbytes`/`peft`/`trl`/llama.cpp/`huggingface_hub` code directly on `agents/agent3-infra` (PR #15), reviewed and fixed (PR #16 — a benchmark crash-handling bug, the same `dtype` deprecation, an unneeded `trust_remote_code=True`, an `fp16`/`bf16` mismatch, a stale `trl` API call, and stale dependency pins). 78/78 tests passing repo-wide.

**Update 2026-09-15 (later)**: grading the baseline found ~24% (17/72) of `manual_review` outputs had collapsed into pure repeated-`#` runs — the base model latching onto the `### Instrução/### Resposta` template markers under greedy decoding. Fixed (PR #17): `repetition_penalty` 1.1→1.3 + added `no_repeat_ngram_size: 3` to `configs/inference.yaml`. Re-ran the baseline with the fix: degenerate rows dropped to 1/72 (and that one is chord-notation free-association, not a repetition loop). 79/79 tests passing. This reset all grading progress (27/72 done on the old file) — T025 is back to 0/72, but now grading clean output instead of garbage. Next: T025 (grade the 72 `manual_review` rows in the regenerated `eval/results/baseline.jsonl`), then T026 (commit it).
...
- [X] T015 (You) Set up CUDA/WSL2/PyTorch on the RTX 5050 machine per `manaca-local-projeto.md` §10; verify `torch.cuda.is_available()` returns `True` — done via a project `.venv` (native Arch Linux, not WSL2 — the doc's environment section assumed Windows, moot on this machine); torch 2.14.0+cu130 confirms CUDA on the RTX 5050 Laptop GPU, and the full stack (transformers/peft/trl/bitsandbytes/datasets/huggingface_hub) is installed and importable
...
- [X] T024 [US1] (You) Run `python -m src.evaluate --model manaca-1b-base --prompts data/eval/grupo_a_prompts.jsonl data/eval/grupo_b_prompts.jsonl --run-id baseline --out eval/results/baseline.jsonl` on the RTX 5050 — depends on T023 — done: 104 rows, rule-based (grammar_correction + classification) pass rate 0/32, confirming the base model doesn't follow instructions (Acceptance Scenario 2)
- [ ] T025 [US1] (You) Run `src/grading/review_cli.py` against `eval/results/baseline.jsonl` to grade every `manual_review` row; confirm at least one `grupo_a` prompt shows the base model not following the instruction (Acceptance Scenario 2) — **not done**: 0/72 graded (regenerated 2026-09-15 after the generation-quality fix below reset all 27 rows graded so far — re-grading is now on clean output, not degenerate `#` runs)
- [ ] T026 [US1] (You) Commit `eval/results/baseline.jsonl` to `001-manaca-instruct-tuning` (direct commit is fine here — it's your own run output, not agent-authored code needing review) — **not done**: file is untracked; commit once T025's manual grading is done, so the committed baseline is complete rather than partially graded
...
- [X] T033 [US2] (You) Run `python -m src.prepare_dataset`; spot-check the resulting `train.jsonl`/`validation.jsonl` for Portuguese correctness and category balance; approve, or send specific examples back to Agent 1/2 for a follow-up fix commit — done: 4,153 examples (3,738 train + 415 validation), no duplicate ids, no eval-prompt overlap. Approved as-is; `grammar_correction` came in under the 1000/category cap at 523 (fewer matching alpaca-pt-br rows than other categories) — usable for a first pass, worth widening `dataset_filters/grammar_rewriting.py`'s keyword regex later if that category underperforms (FR-005's iteration round)
...
- [X] T038 [US2] (You) Run `python -m src.train_qlora --config configs/train.yaml --dataset data/train.jsonl --run-id qlora-v1` on the RTX 5050 — depends on T033, T037 — done: 234 steps (2 epochs, 3738 examples), 10m39s wall time, loss 4.157→1.826 (train_loss avg 2.106), mean_token_accuracy 0.41→0.64. Adapter saved to `adapters/qlora-v1/` (config confirms `base_model_name_or_path: menezesbruno/manaca-1b-base`, `lora_alpha: 32`, matching `configs/train.yaml`)
- [X] T039 [US2] (You) Run `python -m src.evaluate --model manaca-instruct-pt --adapter adapters/qlora-v1 --prompts data/eval/grupo_a_prompts.jsonl data/eval/grupo_b_prompts.jsonl --run-id qlora-v1 --out eval/results/qlora-v1.jsonl` — done: 104 rows. rule_based: classification 0%→25% (improved), grammar_correction still 0% — see open methodology question below before treating this as a real capability failure
- [X] T040 [US2] (You) Run `python -m src.evaluate --model manaca-1b-instruct --prompts data/eval/grupo_a_prompts.jsonl data/eval/grupo_b_prompts.jsonl --run-id official-instruct --out eval/results/official-instruct.jsonl` (the official comparison model, FR-004) — done: 104 rows. rule_based: classification 25%, grammar_correction 0% (same pattern as qlora-v1) — 144 manual_review rows pending grading across both files (T041)

**Resolved (2026-09-15)**: manually read through all 32 grammar_correction outputs (16 per model). Confirmed the edit-distance scorer was hiding real signal — qlora-v1 had ~31% genuine grammar fixes (correct agreement, paraphrased content), the official model ~19%, both misreported as 0%. Decision: `grammar_correction` switched from `rule_based` to `manual_review` (PR #18) — `src/schema_validation.py`'s `RULE_BASED_CATEGORIES` is now `{"classification"}` only. All three eval results files (`baseline.jsonl`, `qlora-v1.jsonl`, `official-instruct.jsonl`) had their 16 `grammar_correction` rows reset to `manual_review`/`score: null` for re-grading — `baseline.jsonl`'s other 72 already-graded rows were untouched. This adds 16 rows/file to what T041 needs to grade (88 total per file now, not 72).
- [X] T041 [US2] Grade all 192 pending `manual_review` rows across `baseline.jsonl` (16), `qlora-v1.jsonl` (88), `official-instruct.jsonl` (88) — done via an AI-assisted first pass (Claude) at the author's request, against the same rubric `baseline-review.md` used; full writeup in `eval/results/qlora-v1-vs-official-review.md`
- [X] T042 [US2] Aggregate by `(model, task_category)` — done: **SC-002 missed on all 5 categories** (best: summarization 31.2%, vs. the 70% target), **SC-003 exceeded** (qlora-v1 improved 16.7%→50.0% on the forgetting-check, no forgetting at all). qlora-v1 beats official-instruct on 4/5 instructed categories; official only wins on grupo_b (70.8%) and shows heavy over-refusal on rewriting (14/16 explicit safety refusals for benign requests)
- [X] T043 [US2] Iteration round executed as `qlora-v2` (FR-005): `num_epochs` 2→3, `grammar_correction` filter widened (523→765 examples; `CohereLabs/aya_dataset` investigated as an alternative and rejected — only 54/8,997 PT rows matched any of the 5 categories). Result: improved on 4/6 dimensions (rewriting +9.4pp, summarization +3.2pp, simplification +3.1pp, grupo_b +6.2pp), tied on 2, no regressions — but **SC-002 still missed on all 5 categories** (best: 34.4%). Per FR-005's one-iteration cap, **qlora-v2 is now final** — the SC-002 shortfall is a documented known limitation (`eval/results/qlora-v2-review.md`, to be reflected in `MODEL_CARD.md`), not something to keep iterating on
- [X] T044 [US2] Commit the final `eval/results/qlora-v2.jsonl` and `eval/results/official-instruct.jsonl` — done (both committed alongside `baseline.jsonl`, `qlora-v1.jsonl` kept for the historical record)
...
- [X] T049 [US3] (You) Run `src/merge_adapter.py` + `src/quantize.py` on the final `qlora-v2` adapter to produce `models/gguf/manaca-instruct-pt-Q4_K_M.gguf` and `-Q5_K_M.gguf` — **done**: also `-f16.gguf` (3.4GB) as the intermediate. Hit and fixed two real bugs along the way: `AutoTokenizer.save_pretrained()` silently dropping `tokenizer.model`/`special_tokens_map.json` (PR #21), and no local `cmake`/`sudo` to build llama.cpp from source (worked around with pre-built CUDA 13.3 binaries from `ggml-org/llama.cpp` release `b10985`). Sizes: Q4_K_M 1.06GB, Q5_K_M 1.23GB
- [X] T050 [US3] (You) Run `src/benchmark.py --machine rtx-5050` on the RTX 5050, writing `benchmarks/rtx-5050.jsonl`; confirm `stalled_or_crashed: false` — **done** for both `Q4_K_M` and `Q5_K_M`, `stalled_or_crashed: false` on both. Found and fixed two measurement bugs first (PR #23): `ram_mb` was reading the parent Python process's RSS (`RUSAGE_SELF`) instead of the llama-cli child's, because GNU `time` isn't installed here — switched to `RUSAGE_CHILDREN`; `vram_mb` was sampled from `nvidia-smi` *after* the short-lived llama-cli process had already exited and freed its VRAM — switched to polling while it runs. Corrected results: Q4_K_M 86.8 tok/s / 1306MB VRAM / 1360MB RAM; Q5_K_M 78.0 tok/s / 1454MB VRAM / 1524MB RAM
- [ ] T051 [US3] (You) Physically run `src/benchmark.py --machine dell-g3` on the Dell G3 (GTX 1050, 4GB VRAM), writing `benchmarks/dell-g3.jsonl`; confirm `stalled_or_crashed: false` and record the real tokens/sec honestly even if it comes in below the ~70 tok/s typical-downloader target (SC-005 applies to a typical modern GPU, not necessarily this machine — see spec.md's Edge Cases) — **deferred indefinitely** (2026-09-17, author's decision): no access to the second machine and no plan to get it; `MODEL_CARD.md` states that no low-VRAM/CPU number is claimed. Not a blocker for T058
- [~] T052 [US3] (You) Commit `models/gguf/*.gguf` (or Git LFS pointers, if the files are large — see note below) and `benchmarks/*.jsonl` to `001-manaca-instruct-tuning` — **partially done**: `benchmarks/rtx-5050.jsonl` committed; GGUF binaries deliberately *not* committed (kept gitignored — 1-3.4GB, distributed via Hugging Face per FR-009/Phase 6, not this repo, per the large-files note below). Waiting on T051 for `benchmarks/dell-g3.jsonl` before closing this out
...
- [X] T058 [US4] (You) Run `src/publish.py` against your own Hugging Face token to push the repository — this is the one genuinely hard-to-reverse action in the whole feature (a public release), so treat it as a deliberate final sign-off, not a routine script run — depends on T044, T052, T057 — **unblocked 2026-09-17 by feature 002** (`eval/results/adoption-decision.md`, PR #30): publish the merged **`qlora-v3b`** adapter (`adapters/qlora-v3b` → `models/merged/manaca-instruct-pt`, GGUFs `models/gguf/manaca-instruct-pt-{Q4_K_M,Q5_K_M}.gguf`, training manifest `runs/qlora-v3b.manifest.json`) with the generation settings in `configs/inference.yaml` (`repetition_penalty 1.1`, `no_repeat_ngram_size 0`) — the exact configuration that was evaluated and adopted under FR-020 of 002. `MODEL_CARD.md` is complete except `<FILL: repo-id>`. **done 2026-09-17 by the author**: `python -m src.publish … --repo-id loizlabz/manaca-instruct-pt` → https://huggingface.co/loizlabz/manaca-instruct-pt (public, `license:cc-by-nc-4.0`; 5.75 GB: safetensors + tokenizer + generation_config with the evaluated settings, `gguf/` Q4_K_M + Q5_K_M only, README = MODEL_CARD.md). Verified from a fresh HF cache: both model-card snippets run verbatim against the Hub copy and answer "o documento foi enviado ontem." (transformers with and without generate() args; llama-completion on the downloaded GGUF, sha256 identical to the local file)
...
- [X] T061 (You) Run the full `quickstart.md` validation end-to-end as a final sign-off — this is also the SC-007 lifecycle-completeness check (every stage executed and documented at least once) — **done 2026-09-17** — every stage of quickstart.md has been executed at least once, with its evidence in the repo: US1 baseline (`eval/results/baseline.jsonl`, frozen sha `8e07ebfc…`); US2 dataset prep (`data/dataset_report.{md,json}`, fingerprints reproducible across runs), training (`runs/qlora-v3{a,b,c}.manifest.json`, plus the 001 runs qlora-v1/v2), evaluation of all three models under one blind protocol (`eval/results/final-table.md`; base and v1 keep their earlier non-blind grades, marked as such); US3 merge + quantization (`models/merged/manaca-instruct-pt`, Q4_K_M/Q5_K_M GGUF) and the RTX 5050 benchmark (`benchmarks/rtx-5050.jsonl`, `source_run_id: qlora-v3b`) — the Dell G3 benchmark was deferred indefinitely (T051), so SC-007's "both benchmarks" is met for one machine only, by decision; US4 publish (https://huggingface.co/loizlabz/manaca-instruct-pt) with both usage snippets verified against the Hub copy. `pytest tests/ -q`: 202 passed, 0 skipped. Numeric thresholds: FR-004's 70% per-category target was not reached (documented in `MODEL_CARD.md`); feature 002's FR-020 rule is what selected the published model. T059 (a stranger follows the model card) remains the author's human test
