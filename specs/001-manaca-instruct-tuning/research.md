...
**Update (2026-09-15), grammar_correction moved to manual_review**: Running the real qlora-v1 and official-instruct evaluations exposed exactly the false-negative failure mode flagged above, at scale — both models scored a *literal* 0% on grammar_correction under the ≥0.9 edit-distance threshold. Reading all 32 outputs by hand found this was largely a scoring artifact, not a true 0%: qlora-v1 produced genuinely correct grammar fixes (e.g. "os menino foi na escola ontem" → "os meninos foram à aula hoje" — correct subject-verb agreement) that scored 0 purely because the model paraphrased content (escola→aula) alongside the fix, landing at ratio 0.787. Manual reading found ~31% real fixes for qlora-v1 and ~19% for the official model — the edit-distance approach couldn't separate "good fix, different phrasing" from "off-topic failure," since both land at low similarity to one canonical reference. `grammar_correction` is now `manual_review` (`src/schema_validation.py`'s `RULE_BASED_CATEGORIES` is just `{"classification"}`); `classification`'s exact-match approach was unaffected by this issue since its label set is small and closed, so a single "expected" value is actually sufficient there.
